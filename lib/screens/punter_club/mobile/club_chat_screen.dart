...
/// Group chat screen: connects to WebSocket, shows messages, supports send/edit/delete and typing.
...
class _GroupChatScreenState extends State<GroupChatScreen> {
  ...
  //* At most one own message selected (long-press); drives top-bar Edit / Delete.
  ...
  ClubChatMessageModel? getSelectedMsgData(PuntClubProvider provider) {
    if (_selectedMessageId == null) return null;
    final list = provider.chatMessages;
    if (list == null) return null;
    for (final m in list) {
      if (m.messageId == _selectedMessageId) return m;
    }
    return null;
  }
        ...
        builder: (context, provider, child) {
          ...
          return StreamBuilder<ChatConnectionState>(
            ...
            builder: (context, connSnapshot) {
              ...
              if (_selectedMessageId != null &&
                  getSelectedMsgData(provider) == null) {
                            ...
                            :
                              ...
                              ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  final isOwn = provider.isMyMessage(msg);
                                  final isSelected =
                                      _selectedMessageId == msg.messageId;
                                  Widget bubble = ClubChatMessageBubble(
                                    message: msg,
                                    isOwnMessage: isOwn,
                                  );
                                  if (_selectedMessageId != null) {
  ...
  Widget goToBottomButton() {
    return Padding(
      padding: EdgeInsets.only(bottom: 70.w, right: 18.w),
      child: Material(
    ...
    final saved = await showDialog<bool>(
      ...
      builder: (ctx) => ZoomIn(
        child: AlertDialog(
          ...
          title: Text(
            'Edit message',
            style: semiBold(color: AppColors.primary, fontSize: 18.sp),
          ),
          content: TextField(
            controller: controller,
            style: regular(color: AppColors.black, fontSize: 16.sp),
          ...
          child: Row(
            children: [
              ...
              if (sel != null) ...[
                ...
                GestureDetector(
                  onTap: () => _showDeleteDialog(context, provider, sel),
                  child: Icon(
                    Icons.delete,
                    color: AppColors.red,
                    size: (context.isMobileWeb) ? 36.w : 26.w,
...
/// Shared [AlertDialog] action style (leave group, edit message, etc.).
Widget myActionButtonTheme({
  ...
  return TextButton(
    ...
    child: Text(
      title,
      style: regular(color: effectiveColor, fontSize: 16.sp),
    ),
  );
}
